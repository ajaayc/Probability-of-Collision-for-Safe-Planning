...
\section{Method}
We propose a sampling-based method to estimate the probability of collision for an arbitrary motion plan. Our method adopts ideas from the the approach proposed by Patil, van den Berg, Alterovitz \cite{IEEEhowto:patil}, though their approach is a purely analytical method.

\begin{figure}[!t]
\centering
\includegraphics[width=3.5in]{motion_pic.PNG}
\caption{Propagation and Truncation of Gaussian Distributions. Figure borrowed from Patil, van den Berg, Alterovitz 2012 \cite{IEEEhowto:patil}}
\label{fig_sim}
\end{figure}
