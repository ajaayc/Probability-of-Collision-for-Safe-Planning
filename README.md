...
### Required Software
- **Python 2.7** (required for OpenRAVE compatibility)
...
- `cPickle` - Object serialization (included in Python 2.7 standard library)
...
- Runs 200 Monte Carlo simulations (configurable in the code, line 221)
...
- Runs 200 estimations (configurable in the code, line 221)
...
- **Number of simulations**: Edit `numSimulations` variable in `MCSimulation.py` (line 221)
- **Number of particles**: Edit `numParticles` variable in `MCSimulation.py` (line 164)
- **Number of Gaussians (GMM)**: Edit `numGaussians` variable in `MCSimulation.py` (line 204)
- **Path planning parameters**: Edit distance/angle weights and discretization in `hw2_astar.py` (lines 118-123)
- **Environment**: Change `envindex` in `hw2_astar.py` (line 73) to use different environments
